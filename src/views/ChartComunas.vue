                ...
                ChartVaccin(comuna){
                  ...
                  return{
                    ...
                    datasets:[
                      ...
                      {label:'segunda dosis', borderColor:'#eba434', backgroundColor:'#eba434', fill: false, data:this.secondDoses.comuna[comuna].slice(index)},
                ...
                async created(){
                  ...
                  allLabelsDeaths.forEach(d => {
                    ...
                    if(dayjs(d,"DD-MM-YYYY").get("day") === 1) {
                      ...
                      if (!Object.values(this.dicMonth2).includes(dayjs(d,"DD-MM-YYYY").format("DD-MM-YYYY"))){
                  ...
                  paso.forEach(comuna => {
                    let date = Object.keys(comuna).slice(5).map(date => {return dayjs(date,"YYYY-MM-DD").format("DD-MM-YYYY")})
                    let fases = Object.values(comuna).slice(5).map(i=>{return Number(i)})
                    ...
                    fases.forEach((fase, index) => {
                      if(fase!=currentFase){
                        currentFase = fase
                        this.pasoAPaso[comuna['comuna_residencia']].dateChangePaso.push(date[index])
                        this.pasoAPaso[comuna['comuna_residencia']].numeroDelPaso.push(fase)
                      }
                    })
                    this.pasoAPaso[comuna['comuna_residencia']].dateChangePaso.push(date[date.length-1])
                    this.pasoAPaso[comuna['comuna_residencia']].numeroDelPaso.push(fases[fases.length-1])
                  })
