    ...
    <div>
      <div class="search">
        <input
          ...
          id="comunas"
          name="comunas"
          v-model="comunaResearched"
          @focus="setFocus"
          @blur="setBlur"
          autocomplete="off"
          v-on:keyup.enter="handleSubmit($event)"
          v-on:keyup.down="nextElement()"
          v-on:keyup.up="nextPrevious()"
        />
      ...
      <div class="listSearch" v-if="searchList">
        <ul id="myUL" v-for="comuna in filtered" :key="comuna">
          <li>
            <a
              href="#"
              v-on:mousedown="emitregion($event)"
              :class="{ selectedStyle: selected === comuna }"
              >{{ comuna }}</a
            >
          </li>
        </ul>
...
export default {
  ...
  watch: {
    comunaResearched(value) {
      let results = this.comunaNames.filter((comuna) => {
        return this.dicAccentWithWithout[comuna].toLowerCase().includes(
          value
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .toLowerCase()
  ...
  created() {
    ...
    this.comunaNames.forEach((name) => {
      this.dicAccentWithWithout[name] = name
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
    });
...
#comunas {
  ...
  /*padding: 12px 10px 12px 40px; /* addsome padding */
